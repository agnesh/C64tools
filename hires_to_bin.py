...
# Opens an 8-bit indexed image 320 x 200 pixels in size and converts the image
# into Commodore 64 bitmap data and color data. The first 16 colors are assumed
# to be in the same order as the C64 palette.
#
# Color order:
#
...
#
# The error indicator image will use the first three colors in the palette
# (assumed to be black, white, red).


def main():

    if len(sys.argv) < 3:
        sys.exit('Usage: hirestobin.py [IMGFILE] [BINFILE]')
    else:
        infile = sys.argv[1]
        outfile = sys.argv[2]
